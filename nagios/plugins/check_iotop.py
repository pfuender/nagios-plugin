...
# Third party modules
from iotop.data import TaskStatsNetlink, ProcessList
...
class IotopOptions(object):
    ...
    def __init__(
        self, only=False, batch=None, iterations=None, delay_seconds=1, pids=None,
            users=None, processes=False, accumulated=False, kilobytes=False,
            time=None, quiet=0, profile=False):
...
class CheckIotopPlugin(ExtNagiosPlugin):
    ...
    def __init__(self):
        ...
        super(CheckIotopPlugin, self).__init__(usage=usage, blurb=blurb)
    ...
    def _add_args(self):
        ...
        msg_tpl = (
            "Generate a %s state if the number of processes with an "
            "I/O-delay of 90/50/10 %%%% is higher as the appropriate "
        ...
        self.add_arg(
            ...
            required=True,
            help=(
                "Delay between iterations in seconds of refreshing "
        ...
        self.add_arg(
            ...
            required=True,
            help=(
                "Set the number of iterations before calculating "
    ...
    def __call__(self):
        ...
        msg_tpl = (
            "The %s thresholds must be given in the form "
        ...
        if self.verbose > 1:
            log.debug(
                "Got thresholds: warning: %r, critical: %r.", self.warning, self.critical)
            log.debug(
                "Number of iterations: %d, delay between: %.1f seconds.",
        ...
        msg = (
            "Total %(total)d procs, %(90)d procs with i/o delay >= 90%%, "
    ...
    def get_proc_stats(self):
        ...
        self.iotop_opts = IotopOptions(
            only=True, batch=True, processes=True,
            iterations=self.iterations, accumulated=True)
