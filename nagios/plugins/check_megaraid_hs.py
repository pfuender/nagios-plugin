...
class CheckMegaRaidHotsparePlugin(CheckMegaRaidPlugin):
    ...
    def parse_args(self, args=None):
        ...
        if crit.end is not None:
            msg = (
                "The critical hot spare number must be given with an "
                "ending colon, e.g. '1:' (given %s).") % (crit)
            self.die(msg)
        self._critical_number = crit

        warn = self.argparser.args.warning
        if warn.end is not None:
            msg = (
                "The warning hot spare number must be given with an "
        ...
        if crit.start > warn.start:
            msg = (
                "The warning number must be greater than or equal to the critical number "
                "(given warning: '%s', critical '%s').") % (warn, crit)
            self.die(msg)
    ...
    def call(self):
        ...
        out += "(warning: <%d, critical: <%d)." % (
            self.threshold.warning.start, self.threshold.critical.start)
