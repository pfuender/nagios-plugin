...
OPTIONS:
  -m, --min-paths     Low mark,  less paths per LUN are CRITICAL   [1]
  -o, --ok-paths      High mark, less paths per LUN raise WARNING  [2]
...
sub get_multipath_text {
    ...
    sub mygrep { my ($needle, $skip, @lines) = @_;
	my @new = ();
	...
	return @new;}
	  my @textArray = mygrep('ATA',3,split (/[\n\r]+/, $output));
	return \@textArray;
}
...
sub checkLunLine {
    ...
    # 36006016019e02a00d009495ddbf3e011 dm-2 DGC,VRAID
    elsif ($textLine =~ m/^[0-9a-fA-F]+ \s+ ([\w\-\_]+) \s+ ([\w\_\_]+)/x) {
	$$rCurrentLun = $1;
...
#    elsif ($textLine =~ m/^[0-9a-fA-F]+\s* ([\w\-\_]+) \s+ ([A-T]{3},[0-9A-Z])+/x) {
#    $$rCurrentLun = $1;
#    report("ATA (1) LUN $$rCurrentLun found", $E_OK);
	}
    else {
